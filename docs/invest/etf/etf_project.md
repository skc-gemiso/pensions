...
- 경로: `/invest/etf`
- 상태: **구현 완료** (stock_analysis → pensions 이전 완료)
...
| 기능 | 설명 |
|------|------|
| 수집 이력 테이블 | 기준일, ETF, 수집 시각, 상태, 종목 수, 메모 |
...
| 이전 완료 | pensions 프로젝트 `/invest/etf` 경로로 통합. 수집기 실행은 `lib/etf-collector.ts` Server Action 방식 |
