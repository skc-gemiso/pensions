...
| 분배금 수익율 조회 | `498400` 전용 팝업 — 분배율 요약(월평균=최근 12개월), 현재 잔고 기준 계좌별 예상 분배금, 지급 이력 테이블(13일 기산 보유 잔고·합계 포함) |
| 분배금 추가 | 배당 팝업에서 `t_etf_dividend` 1건 등록. 엑셀 행 붙여넣기로 5개 항목 자동 분리 입력 |
| 분배금 수정 | 지급 이력 행의 `[수정]` → 같은 입력 폼을 수정 모드로 열어 5개 항목(지급기준일 포함) 갱신 |
...
| 퇴직연금 개인화 | 급여 기준은 `PENSION_RET_*` 환경 변수로 이전 완료. 회사 통보 자료 대조는 미해결 |
