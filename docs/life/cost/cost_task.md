...
두 곳 모두 `use_yn = 'Y'` 인 항목만 센다 — 한쪽만 필터가 빠지면 비활성 항목이 섞여 숫자가 갈라진다.

판정은 `page.tsx` 모듈 상단의 공용 `isCardUsage(row)` 하나로 한다.
집계와 행 표시가 같은 함수를 써야 "지출에서 빠졌다는 표시"와 실제 합계가 어긋나지 않는다.

### 제외된 행은 화면에서 드러낸다

지출 합계는 맞는데 **섹션을 눈으로 훑으면 이중으로 세어진다**. 고정이체의 `학원비 240,000`(현대카드)와
신용카드 섹션의 `현대카드 1,420,970`(그 학원비를 포함한 청구액)이 둘 다 온전한 금액으로 보이기 때문이다.
실제로 섹션 단순합(5,237,476) − 지출(4,432,927) = 카드 사용액(804,549) 만큼 벌어진다.

그래서 제외 대상을 세 곳에 표시한다.

| 위치 | 표시 |
|------|------|
| 행 금액 | 회색(`text-gray-400`) + `↩︎다음달` 꼬리표, `title` 에 제외 사유 |
| 섹션 헤더 카드 소계 | `카드 480,000 · 지출 제외` (`SectionTotals` 의 `cardExcluded`) |
| 수입 대비 지출 현황 | `카드 사용액 · 지출 제외 · 다음 달 청구` |

`cardExcluded` 는 고정지출·고정이체·생활비 3개 섹션에만 넘긴다.
신용카드 섹션(`item_type1='4'`)의 소계는 지출에 그대로 들어가므로 붙이지 않는다.
...
- **섹션 합계·카드/현금 소계**: 각 섹션 안의 단순 합이라 위 제외 규칙과 무관하다.
  그래서 위처럼 "지출 제외" 를 병기해 합계와 기준이 다르다는 걸 드러낸다.
