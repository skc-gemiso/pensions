...
| 총 평가금액 | 보유 잔고 × 현재가(`f_stock_amt` 최신값) 합계 |
...
- **네이버 주가 가져오기** 버튼: 보유 전 종목 대상 `sise_day.naver` 증분 수집 → `f_stock_amt` 저장
- **네이버 금융 →** 링크: 선택 종목 네이버 금융 페이지 새 탭 오픈
- 코스피·코스닥 지수 실시간 조회 결과를 헤더에 표시

### 일별 주가 차트 (종목 선택 시)

- Recharts LineChart — `f_stock_amt` 종가 데이터 사용
...
- 기존 데이터 있으면 최근 6페이지(약 60영업일), 없으면 전체 30페이지(약 300영업일) 수집
- 3페이지씩 병렬 요청(배치) → 기존 최신 저장일 도달 시 수집 중단 (증분 방식)
- `f_stock_amt(stock_code, s_date)` UNIQUE KEY 기준 UPSERT
...
- 보유 잔고(`my_stock` net_qty > 0) 종목만 자동 수집

### 현재가 기준

- 현재가는 `f_stock_amt` 테이블의 최신 저장가(`latest_price`) 기반
- 전일 종가(`prev_price`)는 `f_stock_amt` 역순 정렬 2번째 레코드
...
| 리팩터 | 현재가를 `f_stock_amt` 저장 가격 기반으로 변경 (네이버 실시간 API 제거) |
