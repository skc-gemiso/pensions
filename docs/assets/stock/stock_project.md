...
| 내 잔고 기준 이번 달 예상 분배금 | **현재 보유 잔고** 기준 계좌별 잔고·종가·예상 분배금·과세표준액 |
| 지급 이력 테이블 | 지급기준일·실지급일·분배율·분배금액·과세표준액 + 계좌별 분배금 열 + 보유 잔고·합계 |

**내 잔고 기준 이번 달 예상 분배금** — 실적치가 아니라 현재 잔고로 계산한 추정치다.

컬럼: `계좌 | 보유 잔고 | {기준일} 종가 기준 | 예상 분배금 | 과세표준액`
(종가 열 머리글에 `t_stock_amt` 최신 기준일을 그대로 박아 어느 날 종가로 계산했는지 드러낸다)

```
예상 분배금(계좌) = 현재 보유 수량 × 당일 종가 × 월평균 분배율
과세표준액(계좌)  = 현재 보유 수량 × 최근 주당 과세표준액
```

- 수량은 `getHoldings()` 의 `net_qty` (지금 시점 순수량). 지급 이력 테이블의 **13일 기산 수량과 다르다**
- 당일 종가는 `t_stock_amt` 최신값(`latest_price`), 월평균 분배율은 요약 카드와 같은 최근 12개월 평균
- 계좌 행은 `498400` 을 보유한 계좌만 나온다 (`net_qty > 0`)
