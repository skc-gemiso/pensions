# 퇴직연금 — 기술 사양

`app/pension/ret/` 에 있는 퇴직연금 페이지 전체 사양서.

---

## 파일 구조

```
app/pension/ret/
├── page.tsx      클라이언트 컴포넌트 (UI + 도움말)
└── actions.ts    getRetConfig() — PENSION_RET_* 를 화면으로 전달

lib/pension-ret-calc.ts   계산 전체 (순수 함수). /pension/my 와 공용
```

`page.tsx` 가 클라이언트 컴포넌트라 서버 전용인 `lib/settings.ts` 를 직접 import 할 수 없다.
그래서 `actions.ts` 가 한 겹 끼어 있고, `lib/pension-ret-calc.ts` 는 `process.env` 를 읽지 않는다 —
값은 전부 호출자가 인자로 넘긴다.

---

## 근속 진행 현황

### 날짜 계산

입사일·정년은 **공통 프로필 환경 변수**(`PROFILE_*`, `config/.env`)에서 읽는다 (개인연금 화면과 공유).
`getProfile()` 이 실패했을 때만 `FALLBACK_JOIN` / `FALLBACK_RETIRE` 상수를 쓴다.

```typescript
const profile = await getProfile()          // app/actions/profile.ts
const JOIN_DATE   = toDate(profile.join_date)    // 2015-02-23
const retireDate  = toDate(profile.retire_date)  // retire_rule 로 계산된 정년일

const progress = monthDiff(JOIN_DATE, today) / monthDiff(JOIN_DATE, retireDate) * 100
```

`retire_date` 는 `lib/profile.ts` 의 `calcRetireDate()` 결과다.
현재 규정은 `month_end` — 만 60세 생일이 속한 달의 말일이므로 **2034-06-30**.
과거에는 입사 기념일(2034-02-23)로 잘못 잡혀 있었다.

정년 표기는 `retireDate` 의 연·월을 그대로 쓴다 (`2034년 06월`).

---

## 평균임금 — 퇴직금의 출발점

값은 `PENSION_RET_*` 환경 변수에서 온다 (`lib/settings.ts` `retSettingsFromEnv()`).

```
월 평균임금 = 급여명세서 지급액 계 + 연 상여금 ÷ 12
            = 7,140,000 + 9,000,000 ÷ 12
            = 7,890,000원
```

**상여금을 ÷12 로 얹는 이유** — 법정 평균임금은 퇴직 전 3개월 임금총액에 연 상여금의 `3/12` 를
더해 산정한다. 월로 환산하면 `연 상여금 ÷ 12` 다. 명절·연말 300만 + 5월 인센티브 600만 = 연 900만원.

**지급액 계에 무엇이 들어가나** (2026-08 명세서, 총 7,140,000원)

| 항목 | 금액 | 산입 |
|------|------|------|
| 기본급 | 5,465,720 | ○ |
| 연장근로수당 | 1,124,280 | ○ 통상시급 × 주 6h × 4.34 × 1.5 |
| 식대보조금 | 250,000 | ○ 정기·일률 지급 |
| 자가운전보조금 | 200,000 | ○ — 아래 참고 |
| 통신비 | 50,000 | ○ |
| 간식비 | 50,000 | ○ |

> 자가운전보조금은 실비변상 성격이면 임금이 아니지만, 매월 고정액을 일률 지급하면 임금성이
> 인정된다는 판례를 따라 **포함**한다. 빼려면 `PENSION_RET_MONTHLY_WAGE` 를 `6940000` 으로
> 낮추면 된다 — 오늘 기준 실수령이 8,815만 → 8,600만으로 바뀐다.

### 인상은 연도 차가 아니라 **인상월 통과 횟수**로 센다

연봉 인상은 매년 `PENSION_RET_RAISE_MONTH`(5월)에 한 번 반영된다.
`avgMonthlyWageAt(basis, 퇴직일)` 이 기준 명세서 이후 지나온 인상 횟수를 세어 얹는다.

```
지나온 인상 횟수 = passed(퇴직 연·월) − passed(기준 연·월)
   passed(y, m) = y + (m >= 인상월 ? 1 : 0)

평균임금(퇴직일) = 평균임금 + 인상 횟수 × 연 인상액 ÷ 12
```

**연도 차이(`연도 − 기준연도`)로 세면 안 된다.** 지금 설정에서는 기준 명세서가 8월(5월 인상 후),
정년월이 6월(5월 이후)이라 우연히 같은 값이 나오지만, 둘 중 하나만 5월 이전이면 한 번씩 어긋난다.

| 상황 | 인상월 통과 기준 | 연도 차만 셀 때 |
|------|-----------------|----------------|
| 기준 2026-03(인상 전) → 2026.06 퇴직 | 1회 | 0회 — **누락** |
| 기준 2026-08 → 2027.03(인상 전) 퇴직 | 0회 | 1회 — **과다** |

기준 연월과 인상월을 상수로 박지 않고 환경 변수에 둔 이유가 이것이다.

현재 설정에서의 실제 값:

| 퇴직 시점 | 인상 | 평균임금 |
|-----------|------|----------|
| 2026.06 | 0회 | 7,890,000 |
| 2027.06 | 1회 | 8,090,000 |
| 2028.06 | 2회 | 8,290,000 |
| 2029.06 | 3회 | 8,490,000 |

---

## 퇴직소득세 계산 (2023년 개정)

### 계산 흐름

```
1. 퇴직급여 = 월 평균임금 × 재직일수 ÷ 365
   (= 근속 1년당 한 달치 평균임금. 법정 산식 "1일 평균임금 × 30일 × 재직일수/365" 의
    월 단위 근사로, 월을 30.42일이 아닌 30일로 보는 만큼 약 1.4% 크게 나온다)
2. 근속연수 = 재직일수 ÷ 365 를 **올림** (`taxTenureYears()`)
   소득세법 제48조 ① — 1년 미만의 기간은 1년으로 본다
3. 근속연수 공제액 계산
   - 5년 이하: 100만원 × 근속연수
   - 5~10년: 500만원 + 200만원 × (근속연수 - 5)
   - 10~20년: 1500만원 + 250만원 × (근속연수 - 10)
   - 20년 초과: 4000만원 + 300만원 × (근속연수 - 20)
4. 환산급여 = (퇴직급여 - 근속연수공제) ÷ 근속연수 × 12
5. 환산급여 공제
   - 800만원 이하: 전액
   - 800~7000만원: 800만원 + 초과분 × 60%
   - 7000만원 초과: 4520만원 + 초과분 × 55%
6. 과세표준 = 환산급여 - 환산급여공제
7. 산출세액 = 과세표준 × 누진세율
8. 퇴직소득세 = 산출세액 ÷ 12 × 근속연수 (+ 지방소득세 10%)
```

> **근속연수를 반올림하면 세금이 이중으로 과다 계상된다.** 2번에서 근속연수공제가 작아지고
> (연 250만원), 4번에서 나누는 수가 작아져 환산급여가 높은 세율 구간으로 올라간다.
> 재직 14.36년을 14년으로 깎으면 세금이 390만 → 429만원으로 **39만원 늘어난다.**

### 누진세율 구간

| 과세표준 | 세율 | 누진공제 |
|----------|------|----------|
| 1,400만원 이하 | 6% | 0 |
| ~5,000만원 | 15% | 126만원 |
| ~8,800만원 | 24% | 576만원 |
| ~1억 5천만원 | 35% | 1,544만원 |
| ~3억원 | 38% | 1,994만원 |
| ~5억원 | 40% | 2,594만원 |
| 5억원 초과 | 42% | 3,594만원 |

### 실효세율

`taxMan` 은 전 행이 `calcRetirementTax(grossMan, tenureYears)` 하나에서 나온다.
화면에서는 `taxRateAt()` 이 표 행의 값을 그대로 읽어 쓴다.

**근속이 길수록 실효세율이 낮아지지 않는다.** 근속연수공제가 커지는 것보다 퇴직금이
커지는 속도가 빨라 환산급여가 높은 세율 구간으로 올라간다. 다만 그 상승은 완만하다.

| 근속 | 세전 퇴직금 | 퇴직소득세 | 실효세율 |
|------|-------------|-----------|----------|
| 12년 (2026) | 8,960만원 | 268만원 | 2.99% |
| 15년 (2029) | 1억 2,191만원 | 390만원 | 3.20% |
| 16년 (2030) | 1억 3,347만원 | 436만원 | 3.27% |
| 19년 (2033) | 1억 7,058만원 | 594만원 | 3.48% |
| 20년 (2034, 정년) | 1억 8,374만원 | 652만원 | 3.55% |

> 한때 2030~2034년 행의 `taxMan` 이 `grossMan − netMan`(별도 입력값)이라
> 같은 표 안에서 세율이 3.2% → 5.3% → 5.1% 로 튀었다. 산식을 하나로 통일하면서 사라졌다.
>
> 근속연수를 반올림하던 시절에는 여기가 3.43% ~ 3.76% 였다. 올림으로 고치면서
> 전 행의 세금이 약 39만원씩 줄었다.

---

## 퇴직금 커버드콜 운용 시뮬레이션

**IRP·ISA 는 운용 계획이 없어 다루지 않는다.** 퇴직금을 일시금으로 받아
일반 계좌에서 KODEX 200 타겟위클리커버드콜 **100%** 로 운용하는 기준이다.

```typescript
const CC_STOCK_CODE = "498400"        // KODEX 200 타겟위클리커버드콜
const CC_FALLBACK_ANNUAL_RATE = 0.17  // 분배 이력을 못 읽었을 때만
```

### 분배율

하드코딩하지 않고 **실제 지급 이력**에서 구한다 — 개인연금 화면과 같은 데이터다.

```typescript
getEtfDividendHistory(CC_STOCK_CODE)   // app/sim/actions.ts 재사용
  → 연 분배율 = 최근 12회 AVG(dist_rate) ÷ 100 × 12
```

조회 실패 시 `CC_FALLBACK_ANNUAL_RATE` 를 쓰고 분배율 배지에 `기본 추정치` 로 표시한다.

### 세금은 계산하지 않는다

커버드콜 ETF 분배금은 대부분 **파생상품 매매이익**이라 국내 주식형 ETF 기준으로 비과세다.
실제 과세 대상은 주당 과세표준액(`tax_base_amt`)뿐인데, 분배금의 **4~5%** 수준이다.
배당소득세 15.4%를 그 4~5%에만 매기므로 **실효세율이 1%에 못 미친다.**

금융소득종합과세도 마찬가지다. 연 2,000만원 기준을 과세표준액으로 따지면
연 분배금이 **약 4.8억**(평가액 약 28억)이어야 걸린다 — 이 표의 어느 행도 해당되지 않는다.

그래서 세전/세후 구분, 과세 대상 컬럼, 종합과세 배지를 모두 걷어냈다.
재투자도 분배금 전액으로 한다.

> 세금이 없다는 뜻은 아니다. 과표 비율은 회차마다 0%~16%로 흔들리고 운용사가
> 분배 재원 구성을 바꾸면 커질 수 있다. 도움말 `세금` 탭에 이 단서를 남겨 뒀다.

- 원금(수량)을 헐지 않고 분배금만 받는 구조라 수령액이 유지된다

### 시나리오 — 매입 → 재투자 → 수령

```
매입 시점       실수령 퇴직금 전액으로 커버드콜 매입
    ↓           분배금 전액 재투자 (holdMonths 개월)
63세(2037-06)   이후 분배금만 수령 (수량 고정)
```

#### 매입 시점 = 퇴직 시점

DB형(확정급여형)이라 **퇴직 전에는 회사가 적립금을 운용하므로 개인이 손댈 수 없다.**
그 전에 발생하는 분배금도 없다. 퇴직하는 그 시점에 전액 매입하는 것으로 본다.

#### 만 55세 이전 퇴직은 표에서 제외한다

만 55세 미만으로 퇴직하면 퇴직급여가 **IRP 로 의무 이전**되고 만 55세까지 인출할 수 없다.
이 화면은 IRP 를 운용하지 않는다는 전제이므로 시나리오가 성립하지 않아 **행을 만들지 않는다**
(`DB_ACCESS_AGE = 55`, `accessIdx`).

- 결과적으로 시뮬레이션 표는 **2029년 퇴직부터** 시작한다
- 위쪽 `퇴직 시점별 예상 퇴직금` 표는 2026년부터 그대로 둔다 —
  퇴직금을 받는 것 자체는 성립하고, 운용 시나리오만 성립하지 않는다
- **예외**: 상단 `현재 기준 예상 퇴직금` 카드의 `63세 월 분배금` 은 만 55세 매입 기준으로
  계산한 값을 남긴다. "지금 그만두면"을 보여주는 자리라 비워 두면 카드가 어색해진다.
  실현 가능한 시나리오가 아니라 참고값이라는 점을 도움말에 명시한다

수령 개시 나이는 개인연금과 같은 값(`PENSION_PER_PAYOUT_AGE`)을 `getPerConfig()` 로 읽는다.

```typescript
// 주가는 현재가 고정(상승률 0%) → 분배금만큼만 평가액이 늘어난다
grownValue(원금, 연분배율, 개월) = 원금 × (1 + 연분배율/12) ^ 개월

buyIdx     = 행 연도 × 12 + 정년월        // 퇴직 시점 = 매입 시점
필터        buyIdx >= accessIdx           // 만 55세 이전 퇴직 행 제외
holdMonths = payoutIdx − buyIdx           // 행마다 다르다
평가액     = grownValue(실수령 퇴직금, 연분배율, holdMonths)
연 분배금  = 평가액 × 연 분배율
```

- 퇴직 시점을 그 해 정년월로 잡으면 행 간 개월 차이가 정확히 12의 배수가 된다
- 실측(연 분배율 17.13%) 기준

| 퇴직 시점 | 투자 원금 | 매입 시점 | 재투자 | 63세 평가액 | 월 분배금 |
|---|---|---|---|---|---|
| 2029년 | 1억 1,801만원 | 2029.06 | 96개월 | 4억 6,012만원 | **657만원** |
| 2030년 | 1억 2,911만원 | 2030.06 | 84개월 | 4억 2,466만원 | 606만원 |
| 2034년 | 1억 7,722만원 | 2034.06 | 36개월 | 2억 9,520만원 | 421만원 |

**첫 행(만 55세, 2029년)이 가장 크고 이후 단조 감소한다.** 퇴직금은 작아도 재투자 기간이
8년이라 복리가 이긴다. 다만 **포기하는 근로소득이 반영돼 있지 않다** — 2029년에 그만두면
정년까지 5년치 급여를 받지 않는 것이므로, 이 표만 보고 조기 퇴직을 판단할 수 없다.

> 예전에는 2030년이 꼭짓점(667만원)으로 보였는데, 그건 2030~2034년에 별도 값이
> 박혀 있어서 생긴 착시였다. 산식을 통일하니 꼭짓점이 사라지고 단조 감소가 됐다.

---

## 데이터 구조

### `SalaryBasis` — 급여 기준 (환경 변수)

```typescript
export type SalaryBasis = {
  monthly_wage: number   // 급여명세서 지급액 계 (원/월)
  annual_bonus: number   // 연 상여금 (원)
  annual_raise: number   // 연봉 인상 (원/년)
  raise_month: number    // 인상이 반영되는 달 (1~12)
  wage_base_ym: string   // 명세서 연월 'YYYY-MM'
}
export function avgMonthlyWage(b): number            // 지급액 + 상여 ÷ 12
export function avgMonthlyWageAt(b, on: Date): number // + 그날까지 지나온 인상분
```

이 모듈이 내보내는 계산 함수는 다음과 같다.

| 함수 | 쓰는 곳 |
|------|---------|
| `calcRetirementTax(grossMan, tenureYears)` | 퇴직소득세 (2023 개정) |
| `taxTenureYears(tenureDays)` | 세법상 근속연수 — 올림 |
| `calcCurrentSeverance(monthlyWon, tenureDays)` | 그 시점 기준 퇴직금 `{ grossMan, taxMan, netMan, tenureYears }` |
| `buildRetirementRows(...)` | 퇴직 시점별 표 (`/pension/ret`) |
| `grownValue(원금, 연분배율, 개월)` | 재투자 후 평가액 |
| `calcWithdrawScenario(...)` | 중도인출 참고 시나리오 (`/pension/my` 전용) — 산출은 [my_task.md](../my/my_task.md) 참고 |

타입은 `lib/pension-ret-calc.ts` 가 갖고, `lib/settings.ts` 는 `RetSettings = SalaryBasis` 로
별칭만 둔다 — 화면과 서버가 같은 모양을 주고받게 하려는 것이다.

### 삭제된 `USER_PROJECTIONS` — 되살리지 말 것

2030~2034년만 별도 값이 박혀 있었다.

```typescript
2030: { salaryMan: 9_992,  grossMan: 14_800, netMan: 14_200 }
// ... 2034: { salaryMan: 10_952, grossMan: 17_600, netMan: 16_700 }
```

원본 커밋(`4ae7346`)의 주석은 **"사용자 제공 예상 데이터"** 였는데, 이후 리팩터링에서
"회사 사전 계산값"·`isConfirmed: true`·"확정"으로 격상돼 있었다. **회사에서 받은 자료는 없다.**

값 자체도 앞뒤가 맞지 않았다. `grossMan` 에서 평균임금을 역산하면 이렇다.

| 연도 | 세전 | 재직연수 | 역산 월 평균임금 |
|------|------|----------|-----------------|
| 2030 | 14,800만 | 15.36년 | 964만 |
| 2032 | 16,200만 | 17.36년 | 933만 |
| 2034 | 17,600만 | 19.36년 | **909만** |

임금이 오르는데 역산값은 줄어든다. 세전이 **매년 정확히 700만원씩** 느는데,
`평균임금 × 근속연수` 형태라면 근속과 임금이 같이 늘어 증가폭도 커져야 한다.
재현할 수 없는 규칙이었다는 뜻이다.

지우면서 한꺼번에 사라진 것들:

- 2029→2030 점프 (1억 1,377만 → 1억 4,800만)
- `예상 연봉` 열이 2030년에 거꾸로 줄던 현상
- 실효세율이 구간마다 다른 방식으로 나오던 문제 (3.2% → 5.3% → 5.1%)
- `isConfirmed` 분기와 `monthlyWageMan: null`

### `RetirementRow`

```typescript
export type RetirementRow = {
  year: number
  tenureYears: number      // 퇴직소득세 공제용 근속연수
  monthlyWageMan: number   // 월 평균임금 (만원) — 그 행의 퇴직금을 만든 값
  grossMan: number         // 세전 퇴직금
  netMan: number           // 세후 퇴직금
  taxMan: number           // 퇴직소득세
  isLegal: boolean         // 정년 행이면 true
}
```

`buildRetirementRows(startYear, retireYear, basis, joinDate, retireDate)` 가 만든다.
`retireDate` 의 월·일을 각 연도에 대입해 "그 해 그 날 퇴직"으로 보고 재직일수를 센다.

---

## 도움말

공용 컴포넌트 [components/HelpModal.tsx](../../../components/HelpModal.tsx) 를 쓴다.
사용법은 [environment.md](../../environment.md) 공용 도움말 절 참고.

| 위치 | variant | 탭 |
|------|---------|-----|
| 페이지 제목 | `page` (24px) | 계산 전제 / 퇴직소득세 / ⚠️ 한계와 주의 |
| 근속 진행 현황 | `section` (16px) | 계산 방법 |
| 현재 기준 예상 퇴직금 | `section` | 값 설명 / 읽는 법 |
| 퇴직 시점별 예상 퇴직금 | `section` | 컬럼 설명 / 읽는 법 |
| 퇴직금 커버드콜 운용 시 분배금 | `section` | 이 표가 뭔가요 / 컬럼 설명 / 세금 / ⚠️ 주의 |

### 설명은 전부 도움말 안에 둔다

화면 본문에는 **인라인 주석 블록을 두지 않는다.** 표 하단 `• …` 목록과 페이지 맨 아래
`계산 가정 및 주의사항` 카드가 있었으나, 설명이 길어져 표를 읽기 어려워지므로 전부
도움말로 옮기고 삭제했다. 본문에는 카드·표 헤더의 한 줄 요약만 남긴다.

- 페이지 도움말: 화면 전체 전제(DB형·평균임금·연봉 인상·IRP 미운용) + 퇴직소득세 산식 + 한계
- 카드 도움말: 그 카드의 값·컬럼과 읽는 요령. 중복 설명은 페이지 도움말로 넘긴다
- `세금` 탭은 계산에 넣지 않은 이유(실효세율 1% 미만, 종합과세 사실상 무관)와
  그 전제가 깨질 수 있는 조건을 설명한다
- 분배율·재투자 배수처럼 **조회 시점에 따라 달라지는 값은 본문에 그대로 넣는다**
  (`{(ccAnnualRate * 100).toFixed(1)}%` 식). 하드코딩하면 실제 화면과 어긋난다
- 각 탭은 박스 2~4개를 넘기지 않는다 — 다 읽히지 않으면 없느니만 못하다

---

## 주요 컴포넌트

### 근속 진행 바

- `START_DATE` ~ 정년 기준 진행률 시각화
- Tailwind `w-[{n}%]` 동적 스타일

### 퇴직금 요약 카드

- 현재 시점 기준 세전/세후/세액 3개 수치

### 연도별 퇴직금 테이블

- 2026~2034 행별: 연도 / 근속연수 / **월 평균임금** / 세전 / 퇴직소득세 / 세후
- 전 행이 같은 산식이라 행 간 증가폭을 그대로 비교해도 된다

### 커버드콜 분배금 시뮬 테이블

- 상단: 운용 기준 카드 (커버드콜 100% · 연 분배율) + 만 55세 제외 사유 문구
- 2단 헤더로 시간 흐름을 나눠 배치
  - `매입 후 N세까지 재투자` — 투자 원금 / 매입 시점 / 재투자 개월 / N세 평가액
  - `N세부터 매달 수령` — 월·연 분배금, 앰버 배경
- 만 55세 이전 퇴직 행은 아예 렌더링하지 않는다 (`ccRows` 단계에서 필터)

---

## 알려진 제약 사항

- 평균임금이 **한 장의 명세서** 기준이다 — 연장근로수당(월 112만원)이 그 달 근무에 따라
  변하는데 고정으로 본다. 실제 퇴직금은 퇴직 직전 3개월로 산정된다
- 퇴직금을 `평균임금 × 재직일수 ÷ 365` 로 계산한다. 법정 `1일 평균임금 × 30일` 대비
  약 1.4% 크게 나온다 (월을 30.42일이 아닌 30일로 보는 차이)
- 주가 변동을 반영하지 않는다 — 분배율만 곱하므로 평가액 하락 시 분배금도 함께 준다
- 분배금 세금을 계산하지 않는다 — 실효세율 1% 미만이라 무시했다.
  과표 비율이나 세법이 바뀌면 다시 넣어야 한다
- 세율 개정 시 코드 직접 수정 필요
