...
import { getVolumeChangeTop, getStockSeries } from "../../actions"
...
type TopItem = { ticker: string; name: string; location: string; first_shares: number; last_shares: number; shares_change: number; pct_change: number }
...
export default function VolumeChangePage() {
  ...
  useEffect(() => {
    setLoading(true)
    getVolumeChangeTop(etf, koreaOnly ? "KR" : null, period === 9999 ? null : period).then((d) => {
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto">
        <h1 className="text-xl font-bold text-gray-900 mb-1">수량 변동 분석</h1>
        <p className="text-sm text-gray-500 mb-4">선택 기간 내 보유 수량 변동폭이 가장 큰 종목을 분석합니다.</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-5">
            <div className="px-4 py-3 border-b border-gray-100">
              <h3 className="text-sm font-semibold text-gray-800">
                수량 변동폭 TOP 20 <span className="text-xs font-normal text-gray-500 ml-1">행을 클릭하면 상세 조회</span>
              </h3>
            </div>
            <div className="overflow-x-auto">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                  <tr>
                    {["#", "티커", "종목명", "시작 수량", "현재 수량", "변동수량", "변동(%)"].map((h, i) => (
                ...
                <tbody className="divide-y divide-gray-100">
                  {top.map((t, idx) => (
                    <tr key={t.ticker}
                      ...
                      <td className="px-4 py-2.5 text-right text-gray-700">{fmt(t.first_shares, 0)}</td>
                      <td className="px-4 py-2.5 text-right text-gray-900 font-medium">{fmt(t.last_shares, 0)}</td>
                      <td className={`px-4 py-2.5 text-right font-medium ${Number(t.shares_change) > 0 ? "text-red-600" : "text-blue-600"}`}>
                        {Number(t.shares_change) > 0 ? "+" : ""}{fmt(t.shares_change, 0)}
                      </td>
              ...
              <>
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-4">
                  <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2.5">
                    ...
                    <span className={`text-xs font-medium ml-auto ${Number(selected.shares_change) >= 0 ? "text-red-600" : "text-blue-600"}`}>
                      수량변동 {Number(selected.shares_change) > 0 ? "+" : ""}{fmt(Number(selected.shares_change), 0)}주
                    </span>
                  </div>
                  {(() => {
